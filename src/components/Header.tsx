...
const Header = () => {
  ...
  return (
    <header className="fixed top-0 left-0 right-0 z-[100] transition-all duration-300">
      {/* Announcement Bar */}
      <div className={`bg-black text-white transition-all duration-300 overflow-hidden flex items-center justify-center ${scrolled ? "h-0" : "h-10"
        }`}>
          ...
          <div className="flex items-center justify-between h-14">
            ...
            <Link to="/" className="flex items-center gap-3 group">
              <img
                ...
                className={`transition-all duration-300 object-contain ${scrolled ? "h-10" : "h-16 md:h-20"
                  }`}
              />
              <div className="flex flex-col">
                <span className={`font-black tracking-tighter leading-none transition-all duration-300 ${scrolled ? "text-lg text-gray-900" : "text-xl md:text-2xl text-white"
                  }`}>
                  SHREE <span className="text-primary">JEE</span>
                </span>
                <span className={`font-bold tracking-[0.2em] uppercase transition-all duration-300 ${scrolled ? "text-[8px] text-primary" : "text-[10px] text-primary"
                  }`}>
                  Electricals
                </span>
              </div>
            ...
            <nav className="hidden lg:flex items-center gap-8">
              {navItems.map((item) => (
                <Link
                  key={item.name}
                  to={item.href}
                  className={`text-sm font-bold uppercase tracking-wide transition-colors relative group ${scrolled ? "text-gray-700 hover:text-primary" : "text-white/90 hover:text-white"
                    }`}
                >
                  {item.name}
        ...
        <div className="p-6">
          ...
          <nav className="flex flex-col gap-6">
            ...
            <a
              ...
              className="inline-flex items-center gap-2 text-xl font-bold text-primary border-2 border-primary rounded-2xl px-5 py-3 hover:bg-primary hover:text-white transition-all"
            >
              <Star className="w-5 h-5 fill-primary group-hover:fill-white" />
